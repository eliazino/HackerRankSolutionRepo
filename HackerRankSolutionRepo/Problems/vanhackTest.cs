namespace HackerRankSolutionRepo.Problems {
    ...
    public class Challenge {
        ...
        public static int MonthlyCharge(string month, Subscription subscription, User[] users) {
            ...
            var payingCustomer = users.ToList().Find(F=>F.DeactivatedOn >= startDate && F.ActivatedOn <= startDate && F.CustomerId == subscription.CustomerId);
            
            if (payingCustomer is null)
                return 0;
            int daysInMonth = DateTime.DaysInMonth(startDate.Year, startDate.Month);
            double totalDayCents = (double)subscription.MonthlyPriceInCents /(double)daysInMonth;
            double totalRate = 0;
            for(int i=1; i <= daysInMonth; i++) {
                startDate = DateTime.Parse($"{month}-{i.ToString().PadLeft(2, '0')}");
                int numActive = users.ToList().Count(F => F.DeactivatedOn >= startDate && F.ActivatedOn <= startDate);
                totalRate += (double)numActive * totalDayCents;
            }
            /*int totalDays = GetDayBounding(startDate, payingCustomer.ActivatedOn, payingCustomer.DeactivatedOn);*/
            return (int)Math.Ceiling((double)(totalRate));
        }
